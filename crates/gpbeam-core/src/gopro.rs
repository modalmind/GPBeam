...
pub fn is_gopro_card(vol_root: &Path) -> bool {
    ...
    let Ok(rd) = std::fs::read_dir(&dcim) else {
        return false;
    };
...
pub struct GoProVersion {
    #[serde(rename = "info version", default)]
    pub info_version: String,
    #[serde(rename = "firmware version", default)]
    pub firmware_version: String,
    #[serde(rename = "wifi mac", default)]
    pub wifi_mac: String,
    #[serde(rename = "camera type", default)]
    pub camera_type: String,
    #[serde(rename = "camera serial number", default)]
...
pub fn model_family(fw_version: &str) -> Option<&'static str> {
    ...
    Some(match prefix {
        ...
        "H23" => "HERO12",
        "H24" => {
            return match fw_version.split('.').nth(1) {
                Some("01") => Some("HERO13"),
                Some("02") => Some("MAX2"),
...
impl MediaKind {
    pub fn is_proxy(self) -> bool {
        matches!(self, MediaKind::LowResProxy)
    }
    pub fn is_thumbnail(self) -> bool {
        matches!(self, MediaKind::Thumbnail)
    }
    pub fn is_photo(self) -> bool {
        matches!(
            self,
            MediaKind::Photo | MediaKind::RawPhoto | MediaKind::Burst
        )
    }
}
...
pub fn classify(name: &str) -> MediaKind {
    ...
    // Tier 1
    match (stem, ext) {
        (s, "MP4") if s.starts_with("GH") || s.starts_with("GX") || s.starts_with("GG") => {
            return Video
        }
        ...
        (s, "JPG") if s.len() > 1 && s.starts_with('G') && s.as_bytes()[1].is_ascii_digit() => {
            return Burst
        }
        _ => {}
    }
