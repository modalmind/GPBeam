//! Shared leaf helpers for the offload paths:
//!
...
//!   job, atomically. Returns the `imported` row id. Used by BOTH the
//!   filesystem offload (`orchestrator.rs`) and the wired GoPro offload
//!   (`wired/offload.rs`).
//! * [`stream_hash_to_part`] — a REFERENCE implementation of the
//!   stream-into-`.part`-while-hashing contract (append-aware for resume,
//!   BLAKE3 over every on-disk byte). Currently exercised only by its unit
//!   tests: `copy_verified` (copy.rs) and the wired download
//!   (wired/client.rs) each still run their own streaming loop. Consolidating
//!   those two live paths onto this helper is future work.
...
///
/// NOTE: this is currently a REFERENCE implementation with test-only callers —
/// `copy_verified` (copy.rs) and the wired download (wired/client.rs) each
/// implement their own streaming loop today. Consolidating those two live paths
/// onto this helper (so hashing/resume semantics can never diverge) is future
/// work; the export is kept for the unit tests and that consolidation.
...
/// the ledger and (when `cfg.cloud`'s mirror mode is `Auto` or `Manual`, per
/// Contract G3) enqueue a single cloud job — both in ONE ledger transaction, so
/// a failure can never leave the file recorded-but-never-queued (which later
/// scans would skip, silently never mirroring it). Returns the `imported` row
/// id so the caller can stamp cloud status or correlate events.
///
...
pub fn commit_imported(
    ...
    use crate::ledger::CloudJobSpec;

    // Pre-render the remote path so the borrowed CloudJobSpec can reference it.
    let mirrored = cfg
        .cloud
        .as_ref()
        .filter(|c| matches!(c.mirror_mode, MirrorMode::Auto | MirrorMode::Manual))
        .map(|c| {
            (
                c,
                crate::orchestrator::remote_path_for(&c.remote_root, dest_name),
            )
        });
    let job = mirrored.as_ref().map(|(cloud, remote)| CloudJobSpec {
        destination_id: &cloud.destination_id,
        remote_path: remote,
        total_bytes: size,
        card_src,
    });

    ledger.record_imported_with_cloud_job(
        serial,
        name,
        size,
        captured_unix,
...
mod tests {
    ...
    fn fresh_write_streams_hashes_and_reports_progress() {
        ...
        let mut seen = 0u64;
        let (total, hash) = stream_hash_to_part(&mut reader, &part, 0, &mut |c| seen = c).unwrap();

        assert_eq!(total, payload.len() as u64);
        assert_eq!(
            seen,
            payload.len() as u64,
            "progress reaches the on-disk total"
        );
    ...
    fn empty_reader_produces_empty_hash_and_zero_total() {
        ...
        let (total, hash) = stream_hash_to_part(&mut reader, &part, 0, &mut |_| {}).unwrap();
        assert_eq!(total, 0);
    ...
    fn resume_appends_and_hashes_whole_on_disk_file() {
        ...
        assert_eq!(
            total,
            whole.len() as u64,
            "total covers pre-existing + appended bytes"
        );
        assert_eq!(
            seen,
            whole.len() as u64,
            "progress is cumulative over the whole file"
        );
        assert_eq!(
            hash,
            blake3_hex(&whole),
            "hash is over the WHOLE on-disk file"
        );
    ...
    fn commit_records_but_enqueues_nothing_when_no_cloud() {
        ...
        assert!(id > 0);
        assert!(ledger
            .is_imported("C346", "GX010001.MP4", 4096, 1000)
            .unwrap());
        assert_eq!(
    ...
    fn commit_auto_mirror_enqueues_one_job_with_remote_path_and_card_src() {
        ...
        assert_eq!(
            j.remote_path, "GoPro/2026_GX010001.MP4",
            "remote_root + '/' + dest_name"
        );
    ...
    fn commit_manual_mirror_also_enqueues() {
        ...
        assert_eq!(
            ledger.pending_cloud_count().unwrap(),
            1,
            "Manual mirror queues a job"
        );
    }

    #[test]
    fn commit_is_atomic_record_rolls_back_when_enqueue_fails() {
        // Regression for the record-then-enqueue gap: if the cloud-job insert
        // fails, the imported row must NOT survive — otherwise the file is
        // imported-but-never-queued, later scans skip it via the ledger, and
        // it is silently never mirrored.
        let mut ledger = Ledger::open_in_memory().unwrap();
        ledger
            .raw_conn_for_tests()
            .execute_batch("DROP TABLE cloud_jobs;")
            .unwrap();
        let cfg = cfg_with_cloud(MirrorMode::Auto);
        let dest = PathBuf::from("/dest/2026_GX010001.MP4");

        let res = commit_imported(
            &mut ledger,
            &cfg,
            "C346",
            "GX010001.MP4",
        ...
        assert!(
            res.is_err(),
            "enqueue into a missing table must fail the commit"
        );
        assert!(
            !ledger
                .is_imported("C346", "GX010001.MP4", 4096, 1000)
